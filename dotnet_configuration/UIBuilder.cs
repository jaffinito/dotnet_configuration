...
namespace NewRelic.AgentConfiguration
{
    ...
    public class UIBuilder
    {
        ...
        public void Build(bool showAdvanced)
        {
            ...
            var flow = FormElements.Flows.Section(ConfigurationFile.Merged, _toolTipObj);

            BuildAttributes(ConfigurationFile.Merged, flow);
            ConfigPanel.Controls.Add(flow);

            BuildElement(ConfigurationFile.Merged, ConfigPanel);
        ...
        private void BuildElement(Element element, FlowLayoutPanel parentFlow)
        {
            foreach (var elementItem in element.Elements)
            {
                ...
                if (elementItem.MaxOccurs > 1)
                {
                    if(MultiLineCollection.Exists(QualifiedName(elementItem)))
                    {
                        if (elementItem.Attributes.Count > 0)
                        {
                            MultiLineCollection.GetTextBox(QualifiedName(elementItem)).Text += Environment.NewLine + elementItem.Attributes[0].Value;
                        }
                        else
                        {
                            MultiLineCollection.GetTextBox(QualifiedName(elementItem)).Text += Environment.NewLine + elementItem.Value;
                        }
                    }
                    else
                    {
                        if (elementItem.Attributes.Count > 0)
                        {
                            ...
                            MultiLineCollection.Add(QualifiedName(elementItem), "mlt" + elementItem.Name, parentFlow);
                            MultiLineCollection.GetTextBox(QualifiedName(elementItem)).Text += elementItem.Attributes[0].Value;
                        }
                        else
                        {
                            parentFlow.Controls.Add(FormElements.Flows.MultiLineTextBoxFlow(elementItem, _toolTipObj));
        ...
        {
            foreach (Parts.Attribute attribute in element.Attributes)
            {
